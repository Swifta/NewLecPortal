...
public class AllRowsActionsUITxnXX extends DDateFilterUIDesign implements DUIControllable {
	...
	private BeanItemContainer<OutTxn> container;
	private InTxn inTxn;
	private OutTxnMeta outTxnMeta;
	private MTxn mTxn;
	private List<TextField> tFSearchFields = new ArrayList<>(4);
	
	private Logger log = LogManager.getLogger( AllRowsActionsUITxnXX.class.getName() );
	
	public AllRowsActionsUITxnXX( MTxn mTxn, Grid grid, In in, boolean allowDateFilters, PaginationUIController pageC ){
		this.grid = grid;
		this.in = in;
		this.mTxn = mTxn;
	...
	private void setNewPage( int page ){
		...
		inTxn.setPage( page );
		mTxn.searchTxnToday(in, container );
		format();
	...
	private void refreshGridData(){
		...
		Out out = mTxn.searchTxnToday(in, container );
		mTxn.searchTxnMeta(in, outTxnMeta );
	...
	private void setContent(){
		...
		if( this.allowDateFilters ) {
			...
			Out out = mTxn.setTxnMeta( in, outTxnMeta );
			if( out.getStatusCode() != 1 )
				Notification.show( out.getMsg(), Notification.Type.ERROR_MESSAGE );
	...
	private ShortcutListener getSearchShortcutListener( TextField tF, String itemId, BeanItemContainer<OutTxn> container ){
	...
	private TextChangeListenerCustom getTextChangeListner(
			BeanItemContainer<OutTxn> container,
	...
	public class TextChangeListenerCustom implements TextChangeListener{
		...
		TextChangeListenerCustom( BeanItemContainer<OutTxn> container,
			String itemId, TextField tF ){
