	...
	public void attachBtnCSV() {
		this.btnCSV.addClickListener(e -> {
			...
			try {
				...
				// Load data
				BeanItemContainer<AbstractDataBean> c = this.getExportData();
				...
				ex.printStackTrace();
				btnCSV.setComponentError( new UserError( "Data export failed. Please try again/contact support." ) );
				btnCSV.setIcon(FontAwesome.FILE_TEXT);
				btnCSV.setEnabled(true);
			}
	...
	public BeanItemContainer<AbstractDataBean> getExportData() {
		...
		if (isMulti()) {
			...
			while (itr.hasNext()) {
				...
				t.setTransactionNumber(t.getColumn1());
				;
				c.addBean(t);
			}
